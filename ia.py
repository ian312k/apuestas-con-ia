...
def predict_match_dixon_coles(home, away, team_stats, avg_h, avg_a):
    ...
    return h_exp, a_exp, p_home, p_draw, p_away, p_o15, p_o25, p_btts, top_scores
...
with t1:
    ...
    c_g2.metric("Total (xG)", f"{h_exp+a_exp:.2f}") # YA SIN EL DELTA
    c_g3.metric(away, f"{a_exp:.2f}")

    # --- SECCIÓN: MERCADOS DE GOLES (CORREGIDA) ---
    st.markdown("### 📊 Probabilidades de Gol")
    mg1, mg2, mg3 = st.columns(3) # AHORA SON 3 COLUMNAS
